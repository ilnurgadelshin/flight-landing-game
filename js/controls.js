...
export const CONTROLS = {
  ...
  ndInset:      { key: kbd('J'),                              touch: chip('MAP'), pad: (L) => pb(L.L3) },
  chart:        { key: kbd('E'),                              touch: `${chip('❚❚')} → ${chip('APPROACH CHART')}`, pad: (L) => `${pb(L.Menu)} then ${pb(L.Y)}` },
  ndRange:      { key: `${kbd(',')} / ${kbd('.')}`,             touch: `${chip('MAP')}, then tap the map's left / right third`, pad: (L) => pb(L.L3) },
  ndMode:       { key: kbd('K'),                              touch: `${chip('MAP')}, then tap the map's middle`, pad: (L) => `hold ${pb(L.L3)}` },
